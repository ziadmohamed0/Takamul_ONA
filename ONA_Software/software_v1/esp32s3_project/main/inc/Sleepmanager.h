 ...
 *
 *  2. EXT1 GPIO     → STM32_ALERT_GPIO (IO21 — valid RTC GPIO on ESP32-S3)
 *                     STM32 pulls the pin HIGH when there is an alarm
 *                     (sensor out of range / pump fault / etc.)
 *
 *                     NOTE: IO38 was used before but is NOT an RTC GPIO on
 *                     ESP32-S3 — EXT1 wakeup only works on IO0–IO21.
 *                     Reconnect the STM32 alert wire to IO21 on the PCB,
 *                     or change this define to whichever RTC GPIO you use.
 *
 ...
 *  - last_pump_on       last pump status sent to STM32 (persists across sleep)
 *  - last_speed_hz      last pump speed sent to STM32 (persists across sleep)
 *  - last_target_pres   last target pressure sent to STM32 (persists across sleep)
 *  - controls_valid     whether the above control fields have been set at least once
 *
...
struct RtcData {
    ...
    // ── Controls state persisted across deep sleep ─────────────────────────
    // These replace the instance variables (m_last_pump_on etc.) that were
    // reset to zero on every boot, causing the STM32 to miss commands that
    // didn't appear to "change" from the default false/0.0 values.
    bool     controls_valid;    // true once we have received at least one controls row
    bool     last_pump_on;      // last pump on/off state forwarded to STM32
    float    last_speed_hz;     // last pump speed forwarded to STM32 (Hz)
    float    last_target_pres;  // last target pressure forwarded to STM32 (bar)
};
...
public:
    ...
    // EXT1 wakeup on ESP32-S3 only works with RTC GPIOs (IO0–IO21).
    // IO38 is NOT an RTC GPIO — the log showed: "E sleep: Not an RTC IO: GPIO38"
    // Reconnect the STM32 alert wire to IO21 on the PCB.
    // If you use a different RTC GPIO, change this value accordingly.
    static constexpr gpio_num_t STM32_ALERT_GPIO = GPIO_NUM_21;
    ...
    void saveLastControls(bool pump_on, float speed_hz, float target_pres);
private:
