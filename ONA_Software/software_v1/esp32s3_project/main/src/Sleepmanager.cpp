 ...
 *    ↓ SleepManager::saveLastControls()    <- Save last controls to RTC
 ...
 *    - EXT1 GPIO -> STM32 pulled IO21 HIGH (alert/fault)
 * ============================================================================
 */

#include "inc/Sleepmanager.h"
#include "inc/UartBridge.h"      // FIX: needed to call uart.stop() safely
#include "esp_log.h"
#include "esp_sleep.h"
#include "esp_wifi.h"
...
// ─── saveLastControls ─────────────────────────────────────────────────────────
//
// FIX: Saves the last control values forwarded to the STM32 into RTC memory
// so they persist across deep sleep cycles. Without this, the instance
// variables in TelemetryManager (m_last_pump_on etc.) reset to false/0.0 on
// every wakeup, causing commands already in Supabase to appear "unchanged"
// and never be re-sent to the STM32.

void SleepManager::saveLastControls(bool pump_on, float speed_hz, float target_pres)
{
    g_rtc.magic            = RTC_MAGIC;
    g_rtc.controls_valid   = true;
    g_rtc.last_pump_on     = pump_on;
    g_rtc.last_speed_hz    = speed_hz;
    g_rtc.last_target_pres = target_pres;
    ESP_LOGD(Tag, "RTC controls saved: pump=%s speed=%.1f Hz pres=%.2f bar",
...
void SleepManager::configureWakeupSources() {
    ...
    // FIX: Using IO21 which is a valid RTC GPIO on ESP32-S3.
    // EXT1 wakeup requires RTC GPIOs (IO0–IO21 only).
    if (STM32_ALERT_GPIO >= 0) {
...
void SleepManager::shutdownPeripherals() {
    // FIX: Stop UartBridge FIRST — this signals the rxTask to exit and waits
    // for it to fully finish before the UART driver is deleted internally.
    // Without this, uart_driver_delete() was called while rxTask was still
    // blocked in uart_read_bytes(), causing a spinlock_acquire assert crash.
    ...
    // Short delay to ensure log is printed before sleep
...
void SleepManager::prepareAndSleep() {
    ...
    // Note: controls_valid, last_pump_on, last_speed_hz, last_target_pres
    //       are intentionally NOT cleared — they must survive the sleep.
    ...
    // Stop peripherals cleanly (UartBridge first, then WiFi)
    shutdownPeripherals();
